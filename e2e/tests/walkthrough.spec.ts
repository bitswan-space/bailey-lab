...
test('Bailey product walkthrough → manual screenshots', async ({ page }) => {
  ...
  const progressSignature = async (): Promise<string> => {
    ...
    const status = await d.getByText(/Healthy|services? not running|services? restarting|Not deployed yet|Deploying|Building|Pulling|Starting|Preparing|Promoting|Generating|Configuring|Reconciling|Provisioning|Installing|Recording|Updating|updated|never deployed/i).first().textContent({ timeout: 1500 }).catch(() => '');
