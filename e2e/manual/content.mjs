  ...
  chapters: [
    ...
    },
    {
      ...
      sell: [
        ...
        'What it then does is a <strong>rebase, not a snapshot</strong>. Your commits are <em>replayed</em> onto main’s tip — each one arriving as itself, not flattened into a single “overwrite” — with your side winning wherever you and a colleague changed the same lines. <strong>Anything main added that you never touched is kept.</strong> Main’s own commits stay in the history underneath, reachable, because nothing here is ever rewritten. A second option in the same dialog says the blunter thing plainly — <em>make main exactly my version</em> — and drops main’s additions too; it is a separate choice precisely because most people do not mean it. And a conflict no rule can decide (one side deleted a file the other edited) <strong>changes nothing at all</strong> and hands the rebase to the coding agent, exactly as a blocked Sync does.',
      ...
      steps: [
        ...
        'If your version is genuinely the right one and syncing is not what you mean, use <b>Deploy this version, overwriting main…</b> instead — read the list of whose commits it supersedes, choose whether main keeps their untouched additions, and type the process name to confirm.',
