...
export const MANUAL = {
  ...
  chapters: [
    ...
    // ───────────────────────── Inside the workspace ─────────────────────────
    {
      ...
      lede: 'Open a workspace and you land in its dashboard, already working in your own copy — an isolated branch where nothing you do touches main until you choose to. You are never asked to pick one. Everything else about copies — a colleague’s version, a throwaway experiment — sits behind a single Advanced menu.',
      slots: [
        ...
        { id: 'advanced-menu', label: 'Live capture', caption: 'Advanced · the whole copy tree in one menu — see a colleague’s version (expandable to their experiments), or start an experiment off your own copy' },
        { id: 'experiment', label: 'Live capture', caption: 'Inside an experiment · the green banner names it and carries the only two ways out — merge it back into your copy, or discard it. There is no Deploy tab in here.' },
      ...
      sell: [
        ...
        'The two exceptions live under <strong>Advanced</strong>, on the far right. <strong>See a colleague’s version</strong> lists everyone else’s copy — expand one and you also see the experiments hanging off it. Open any of them and you land under an <strong>amber banner</strong>: <em>“You are viewing ⟨name⟩’s copy”</em> (or their named experiment), with one <em>Switch back to my copy</em> button. Nothing is locked down — their copy stays fully usable, because reading a colleague’s work is normal. The banner exists so you can never <em>mistake</em> their copy for yours.',
        'The other half is <strong>experiments</strong>. An experiment is a throwaway branch off <em>your own</em> copy, and you name it by <strong>what you are trying</strong> — “Check vendor VAT-IDs against ARES” — never by a branch name; Bailey commits your copy’s current state, branches from exactly that, and drops you in. A <strong>green banner</strong> reads <em>“You are in an experiment: ⟨title⟩”</em> and carries the only two exits. <strong>Merge back into my copy</strong> fast-forwards the work into your copy, refreshes your copy’s live-dev — and then <strong>discards the experiment and puts you back on your own copy</strong>, because a merged experiment has no reason to exist. (If your copy moved on far enough to conflict, the merge doesn’t guess: it hands the rebase to the coding agent, working inside the experiment.) <strong>Discard experiment</strong> is the other exit and a real delete — the branch, the files and its live-dev containers — behind a dialog that names the unmerged work it would destroy.',
      ...
      steps: [
        ...
        'Open <b>Advanced</b> → <b>Start a new experiment</b> and name what you’re trying; you land in it under the green banner.',
        'Work in the experiment exactly as you would in your copy — same tabs, same live-dev, its own branch.',
      ...
      standards: [
        { code: 'ISO/IEC 27001', clause: 'A.8.31', demand: '<b>Separation of development, test and production.</b> A personal copy — and any experiment branched off it — isolates work-in-progress from main and from every deployed stage; an experiment has no path to main at all.' },
    ...
    },
    {
      ...
      steps: ['Open the <b>Coding Agent</b> tab; start a session against your copy.', 'It builds the automation from the Description; the <b>live-dev</b> preview comes up.', 'Edit files by hand in the <b>Files</b> sub-tab when you need to — e.g. set a worker’s <b>memory_reservation_policy</b> to always-on — and save; the change rides your copy.', 'Click a running automation’s <b>frontend</b> to open it and click through it.', 'Open <b>Requirements &amp; tests</b>; record the rules as runnable specs, then press <b>Deploy</b>.'],
      standards: [
    ...
    },
    {
      ...
      lede: 'Two steps, one direction. <b>Deploy</b> publishes your copy into main and rolls it out to development; the <b>Sync</b> step appears first, and only when it must, to bring main’s new work into your copy before you do. With the diff, the history and a security check standing between you and production.',
      slots: [
      ...
      sell: [
        ...
        'That is what the <strong>Sync</strong> step is: main’s work coming <em>into</em> your copy, and the one place divergence is ever resolved. It appears as the <strong>first</strong> tab of the process — before Description — <em>only</em> while main carries commits your copy lacks, and <em>only</em> on your own copy; the moment you’re level again it disappears, because a step with nothing to do is noise. It shows what is arriving, per business process, and names the actual incoming commits — subject and author — so “what did they change?” is answered before you take it. One button does it: <strong>Pull main into my copy</strong>, which replays your work on top of theirs and refreshes the live-dev of anything that changed. A genuine conflict is not swallowed either: it is handed to the coding agent, in your copy, to finish by hand. Take their work first, see it running in your own preview, then ship — so a deploy is never a merge that quietly buries someone else’s change.',
      ...
      steps: [
        'Open <b>Deploy</b>.',
        'If a <b>Sync</b> tab is showing (or Deploy says “sync first”), a teammate moved main before you: open it, read the incoming commits, and press <b>Pull main into my copy</b> — the tab disappears once you’re level.',
