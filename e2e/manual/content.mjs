  ...
  chapters: [
    ...
    },
    {
      ...
      slots: [
        ...
        { id: 'audit-diff', label: 'Live capture', caption: 'Audit environment · exactly what promoting the frozen image would change in production' },
        { id: 'audit-source', label: 'Live capture', caption: 'Audit environment · the source as deployed to staging, searchable — the version under review, not whatever the workspace moved on to' },
        { id: 'audit-report', label: 'Live capture', caption: 'Audit report · the audit agent read the pinned version and its diff and drafted the findings; the auditor corrects and keeps them' },
      ...
      sell: [
        'Freezing does more than lock the image: it <strong>builds the audit environment</strong>. The auditor gets the source <em>as deployed to staging</em> with a search over it, the <strong>diff against what production is serving right now</strong> — exactly what the promotion would change — and a <strong>coding agent of their own</strong>, in its own temporary container, that can read both and cannot edit either. It drafts the report; the auditor corrects it and keeps it. The report is stored against the image\u2019s content hash, so the evidence and the sign-offs it justifies always name the same image, and unfreezing keeps both.',
