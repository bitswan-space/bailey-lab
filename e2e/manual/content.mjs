  ...
  chapters: [
    ...
    },
    {
      ...
      slots: [
        ...
        { id: 'file-editor-saved', label: 'Live capture', caption: 'File editor · the memory policy edited to always-on and saved — versioned on your copy branch, not on main until Sync & Deploy' },
    ...
    },
    {
      ...
      sell: [
        ...
        'Around the button are the three things you check before shipping: the <strong>Diff</strong> (exactly what becomes main), the <strong>History</strong> (your copy’s and main’s commits, with deploy markers), and the <strong>Supply Chain Security</strong> tab — the precise CVEs of the image this deploy <em>would</em> build.',
        'When a finding doesn’t apply — an unreachable code path, a false positive, a risk already handled elsewhere — mark it <strong>out of scope</strong> from the same tab. The decision is a property of the code, so it is written into the business process’s <strong>source tree</strong> (<code>cve-waivers.yaml</code>, versioned in git) with <strong>who marked it, when, and why</strong>, and rides Sync &amp; Deploy to main alongside the code it concerns. Every out-of-scope CVE is then listed in a dedicated <strong>audit-log</strong> section — on this tab, where a reviewer can <strong>restore</strong> it to in-scope, and read-only on the production <strong>Supply chain</strong> tab, so an auditor sees exactly what was excluded, by whom, and on what grounds. Nothing is buried in config, and no marking is silent.',
      ],
      steps: ['Open <b>Sync &amp; Deploy</b>.', 'Review the <b>Diff</b>, the <b>History</b>, and the <b>Supply Chain Security</b> tab.', 'Mark any non-applicable CVE <b>out of scope</b> — it’s recorded in your source tree with who/when/why.', 'Press <b>Sync &amp; Deploy</b> — dev goes <b>Healthy</b>.', 'Promote when you’re ready.'],
