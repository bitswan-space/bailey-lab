  ...
  chapters: [
    ...
    // ───────────────────────── Inside the workspace ─────────────────────────
    {
      ...
      slots: [
        ...
        { id: 'workspace-settings', afterPara: 1, label: 'Live capture', caption: 'Advanced → Settings (admins only) · the workspace’s own SSH deploy key, ready to copy, and the git remote the workspace is mirrored to — one repository, one folder per process on main, read-only copies/* branches, and the deployment manifests on gitops' },
    ...
    },
    {
      ...
      sell: [
        ...
        'That frontend is not a mock-up you’ll later have to find a host for — it <strong>is</strong> the process’s production web app. The moment it runs, Bailey gives it a real, routable hostname with TLS already in place, on your Bailey server’s own domain; there’s no separate hosting account, DNS zone or certificate for Marek to set up. (Bring your own domain later if you want one — it’s optional, never required. And if you would rather host it elsewhere, that stays open: an admin can mirror the workspace to a git remote of your own, so the source leaves with you.) The backend stays off the public internet entirely, reachable only from the frontend over the workspace’s private network.',
