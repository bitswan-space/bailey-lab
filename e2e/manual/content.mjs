...
export const MANUAL = {
  ...
  promises: [
    ...
    { title: 'Ship the app, not a hosting ticket', body: 'A business process’s frontend and backend run inside Bailey from the moment you create it — a real, TLS-secured hostname on day one, no web host, DNS or certificate to arrange yourself. Point your own domain at it later if you want to, or host the app somewhere else entirely: an admin can mirror the workspace to a git remote of your own, so the source is never locked in. You never have to — which is not the same as cannot.' },
  ],
  ...
  chapters: [
    ...
    // ───────────────────────── Inside the workspace ─────────────────────────
    {
      ...
      slots: [
        ...
        { id: 'advanced-menu', afterPara: 1, label: 'Live capture', caption: 'Advanced · the whole copy tree in one menu — see a colleague’s version (expandable to their experiments on this business process), or start an experiment on it off your own copy' },
        { id: 'experiment', afterPara: 2, label: 'Live capture', caption: 'Inside an experiment · the green banner names the business process it is on and the experiment itself, and carries all four ways out — step back to your copy leaving it running, merge it back, take it wholesale, or discard it. There is no Deploy tab in here.' },
        { id: 'experiment-merge', afterPara: 2, label: 'Live capture', caption: 'Merging back · the experiment’s work fast-forwards into your own copy — and the experiment is discarded in the same move, leaving you on your copy' },
        { id: 'experiment-adopt', afterPara: 3, label: 'Live capture', caption: 'Use this version without merging · the fourth way out of an experiment — your copy becomes it wholesale, and what your copy held is parked as an experiment of its own, named after the moment' },
      ],
      sell: [
    ...
    },
    {
      ...
      sell: [
        ...
        'That frontend is not a mock-up you’ll later have to find a host for — it <strong>is</strong> the process’s production web app. The moment it runs, Bailey gives it a real, routable hostname with TLS already in place, on your Bailey server’s own domain; there’s no separate hosting account, DNS zone or certificate for Marek to set up. (Bring your own domain later if you want one — it’s optional, never required. And if you would rather host it elsewhere, that stays open: an admin can mirror the workspace to a git remote of your own, so the source leaves with you — Ch&nbsp;29.) The backend stays off the public internet entirely, reachable only from the frontend over the workspace’s private network.',
    ...
    },
    {
      ...
      sell: [
        ...
        'A requirement’s <b>status</b> is the agent’s work queue. One you write starts as <b>pending</b> — recorded, not yet tested — and <b>Requirements next</b>, which is how the agent picks what to work on, hands it the first requirement in tree order that is <em>not</em> <b>pass</b>. So <b>pending</b>, <b>fail</b> and <b>retest</b> all read as work to do; only <b>pass</b> takes a requirement out of the queue. You don’t set <b>pass</b> or <b>fail</b> yourself: they come from the work on the requirement — a test run, or the agent recording what it found as it built. What you do is accept a proposal, which moves it into the contract so it gets tested, or send a passing requirement <b>back to be re-checked</b> when you no longer trust it; both are actions on the row. <b>proposed</b> is the one status that means <em>not yet part of the contract</em> — test runs skip those, which is why the agent’s own suggestions land there instead of in the tested set. Worth knowing: <b>pass</b> and <em>covered</em> are not the same thing. Whether a requirement has a test at all is tracked separately — the tab scans for one carrying the requirement’s id — so a requirement can read <b>pass</b> with nothing behind it but a judgement, the agent’s or yours.',
      ],
      steps: ['Open the <b>Coding Agent</b> tab; start a session against your copy.', 'Or skip the typing: press <b>Build automation</b> on the <b>Description</b> tab and it takes your specification from there — it builds from it, the <b>live-dev</b> preview comes up, and it proposes the testable requirements.', 'Edit files by hand in the <b>Files</b> sub-tab when you need to — e.g. set a worker’s <b>memory_reservation_policy</b> to always-on — it autosaves, and the change rides your copy.', 'Click a running automation’s <b>frontend</b> to open it and click through it.', 'Open <b>Requirements &amp; tests</b>; accept or reword the agent’s <b>AI-</b> proposals, add any <b>REQ-</b> of your own, press <b>Write tests</b> to have them covered — then press <b>Deploy</b>.'],
      standards: [
    ...
    },
    {
      ...
      steps: ['Open <b>Deployments</b> and read the pipeline: <b>Development</b>, <b>Staging</b>, <b>Production</b>.', 'Build and test in <b>your own copy</b> — the <b>Development</b> stage runs whatever is on <code>main</code>, so nothing lands there until you <b>Deploy</b> (Ch&nbsp;10), which publishes your copy into main and rolls it out to Development.', 'Once it’s running in Development, <b>promote</b> it to <b>Staging</b> (Ch&nbsp;11) and exercise the app on staging’s own data — never production’s.', 'Have an experienced reviewer sign off before the deliberate promotion to <b>Production</b>.'],
    ...
    },
    {
      ...
      sell: [
        ...
        'Sync is the answer almost every time, and it is the <em>first</em> one offered — but it is not the only honest one, and pretending otherwise is what sends people round the product with raw git. Beside it sits a deliberately quiet <strong>Deploy this version, overwriting main…</strong>. It opens a dialog that <em>reads main live</em> and lists every commit your copy does not have: <strong>what it says, who wrote it</strong>, and a short id to look it up by — these are colleagues, and “main has diverged” hides the one fact that would change your mind. If that list cannot be read, the button stays disabled and the dialog says why; understating the damage is not an option. Confirming means typing the business process’s own name, the same guard rail a production data restore uses.',
        'What it then does has <strong>exactly one outcome</strong>: <strong>main ends up holding exactly what your copy holds</strong> — including <em>dropping</em> anything main added that you do not have. There is no mode to pick and no merge rule to reason about, deliberately: nobody can predict from a label whether a colleague’s unrelated file survives, and a rule that tries leaves main holding a version that exists in nobody’s copy. “Overwriting main” means exactly what it says, and the dialog states the loss before you confirm it.',
    ...
    },
    {
      ...
      sell: [
        'An audit is not a separate world: it happens in a <strong>copy of the version under audit</strong>. Opening one gives the auditor that exact version \u2014 the source the frozen image was built from \u2014 in an ordinary copy, so they read it with the <strong>same coding agent, file explorer and diff</strong> they use every day, and write their findings to a report file that sits alongside it \u2014 kept out of the source, so proposing a fix proposes code and nothing else. Nothing they do there can change what is frozen: a sign-off attaches to the image\u2019s content hash, and a copy is not that image.',
        ...
        '<strong>How an audit is conducted.</strong> The copy opens with a report already scaffolded under four headings, and working through them <em>is</em> the method. <em>What this version changes</em>: start from the diff against what is running in production \u2014 that is the actual surface of risk, and it is one tab away. <em>Risk</em>: what could this do that the last version could not \u2014 new egress, new data, a threshold in code rather than configuration? <em>Verified</em>: what you actually read or ran, in enough detail that someone else could repeat it. <em>Not verified</em>: the honest remainder. An audit that states its limits is worth more than one that implies it checked everything. The <strong>coding agent is in the copy too</strong> and will draft the report for you \u2014 usefully, as a research assistant whose claims you check, because it is your name on them.',
        '<strong>You sign off on the report, not on a text box.</strong> <em>Approve</em> and <em>Request changes</em> sit at the <strong>foot of the report</strong>, where an auditor who has just read to the end of it is looking \u2014 and the two ways out of an audit stand side by side in the top bar, <em>Audit report</em> <strong>or</strong> <em>Propose a new version</em>: pressing one saves the report, reads it back and records it <em>with</em> the verdict. What is stored is the argument, not a thumbs-up \u2014 so a year later, in a review or an incident, the record shows what was checked, what was not, and on what grounds it was released.',
        '<strong>A release\u2019s record closes when it ships.</strong> The moment a version is promoted to production, the sign-offs it went out on are fixed: the promotion writes down <em>which</em> verdicts released it, and that image stops accepting new ones. Looking again is always allowed \u2014 but a second opinion months later cannot rewrite what a past release was approved on, and auditing the next version means deploying a change, which is a new version with an audit of its own.',
      ...
      steps: [
        'As an auditor, <b>Freeze</b> the Staging node to lock the image under review.',
        'Open <b>Audits</b> \u2192 <b>Open the code for auditing</b>: a copy of that exact version.',
        'Read it \u2014 the production diff, the source, the agent \u2014 and write the <b>Audit report</b> tab.',
        '<b>Approve</b> or <b>Request changes</b> on the report itself \u2014 or fix what you found and deploy, which proposes a new version.',
        'Once the policy is met, <b>Promote to Production</b> unlocks.',
      ],
    ...
    {
      num: '29', eyebrow: 'Keep your options open', title: 'Take the source with you',
      lede: 'Everything a business process is — its code, its spec, its CVE waivers, its deployment manifests — lives in git repositories this workspace owns. An admin can mirror them to a git remote of <b>your</b> own, so the source is never locked in: you can clone it, read it, and host the app somewhere else if you ever want to.',
      slots: [
        { id: 'workspace-settings', afterPara: 0, label: 'Live capture', caption: 'Advanced → Settings (admins only) · the workspace’s own SSH deploy key, ready to copy, and the git remote the workspace is mirrored to' },
      ],
      sell: [
        'Bailey runs your processes for you — the frontend, the backend, the databases, the certificates (Ch&nbsp;06). That is a convenience, and a convenience you cannot walk away from is something else. So the workspace will push <strong>everything it holds</strong> to a git remote you control: open <b>Advanced → Settings</b> (admins only) and the <strong>Git remote</strong> card is there. Each workspace generates <strong>its own SSH deploy key</strong>, shown in full with a <b>Copy public key</b> button — add it as a read-write deploy key on GitHub, GitLab or Forgejo, paste the SSH remote back (<code>git@host:org/repo.git</code>; HTTPS is refused, because the key is what authenticates), and press <b>Push now</b>. The key can be rotated from the same card — <b>Retire key and generate a new one</b> — and the card records who rotated it and when.',
        'What lands there is not an export you have to ask for; it is <strong>your workspace, kept current</strong>, in one repository whose shape you can read without any Bailey documentation. <strong><code>main</code></strong> carries one folder per business process, each holding that process’s code exactly as its own main has it, plus a generated <code>README.md</code> that explains the layout to whoever clones it next. <strong><code>gitops</code></strong> carries the deployment manifests — which version each stage is on. <strong><code>copies/⟨name⟩</code></strong> branches mirror every personal copy and experiment, read-only and overwritten on each push, and they disappear when the copy does. Deployment history rides along as <strong><code>deploy/⟨process⟩/⟨timestamp⟩</code></strong> tags. Clone it and you have the code, the spec, the waivers and the record of what ran where.',
        'It is <strong>two-way</strong>, which is what separates a real exit from a backup. Push a commit to that remote from anywhere and it comes back: before the Deploy tab tells you where you stand, Bailey fetches the remote’s main and merges those commits into the process’s own main, then says so in plain words — <em>“New changes from the git remote were pulled into main. Sync your copy before deploying.”</em> A colleague can fix a typo from a laptop with no Bailey access at all and the fix is simply part of the next deploy.',
        'And it is <strong>honest when it cannot do that</strong>. <strong>Bailey never force-pushes main on its own.</strong> If the remote’s main is no longer a fast-forward of what Bailey pushed — someone rewrote history there — or an inbound change genuinely conflicts with work in the workspace, mirroring of main <em>stops</em> and the card and the Deploy tab both say which processes are affected. The way out is a deliberate choice by an admin: <b>Force push to repair</b>, which makes the remote match the workspace again and says what that discards, or <b>Pause the remote</b>, after which nothing is pushed or pulled until someone resumes it. Members see the same banner with the honest instruction to ask an admin. Nothing is silently reconciled, and nothing is silently dropped.',
        'That is the whole promise of this chapter, and it is deliberately unexciting: <strong>you never have to leave, which is not the same as cannot</strong>. The processes keep running on your own server either way; the mirror means the source keeps working without Bailey too.',
      ],
      steps: ['Open <b>Advanced → Settings</b> (admins only) and find the <b>Git remote</b> card.', 'Press <b>Copy public key</b> and add it as a <b>read-write deploy key</b> on your own GitHub, GitLab or Forgejo repository.', 'Paste the <b>SSH</b> remote (<code>git@host:org/repo.git</code>) — HTTPS is not accepted — and save it.', 'Press <b>Push now</b>; <b>Push status</b> then lists each branch and what reached the remote.', 'Clone that repository any time: <code>main</code> for the code, <code>gitops</code> for the manifests, <code>copies/*</code> for everyone’s work in progress.', 'Push a commit to it from outside and it is pulled into main before the next Deploy — Sync your copy and ship it.', 'If main ever diverges, choose on purpose: <b>Force push to repair</b>, or <b>Pause the remote</b> until you have decided.'],
      callout: { kind: 'Why it matters', text: 'An exit that needs a support ticket is not an exit. The mirror is a standing, self-service one: the code, the specs, the CVE waivers and the deployment record are in a repository you already control, in a layout a stranger can read, updated on every deploy. Leaving is a decision you can make without asking us — which is the only version of that promise worth printing.' },
      standards: [
        { code: 'DORA', clause: 'Art. 28(8)', demand: '<b>Exit strategies for ICT third-party arrangements.</b> The workspace mirrors itself — code, deployment manifests and history — to a git remote the financial entity controls, so an exit needs no cooperation from the provider.' },
        { code: 'ISO/IEC 27001', clause: 'A.5.23', demand: '<b>Information security for use of cloud services.</b> The mirror covers the acquisition-to-exit lifecycle: what the service holds is continuously available in a customer-owned repository, in a documented layout.' },
        { code: 'ISO/IEC 27001', clause: 'A.5.30', demand: '<b>ICT readiness for business continuity.</b> A current, independent copy of every business process — code, spec and manifests — keeps running when the platform that produced it does not.' },
      ],
    },
  ],
