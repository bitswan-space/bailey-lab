...
const NAV = [
  ...
  { group: 'Admin', items: [
    ...
    { id: 'sso',       label: 'Single sign-on',   icon: 'key-round' },
  ]},
];
...
// Which handbook artifacts this build actually carries. The handbook is
// generated by the E2E product walkthrough and baked into the console dist by
// the release workflow, so a binary built without that step carries none —
// offering the buttons anyway is a dead link (the daemon can only answer 404).
// The daemon states what it has in the shell HTML:
//   <meta name="bitswan-handbook" content="html,pdf">   (empty = none)
// The SPA and the daemon ship in the same binary, so a missing tag means the
// serving side never declared one: treat that as "no handbook" too, and never
// offer a link we cannot back.
function handbookFormats() {
  try {
    const el = document.querySelector('meta[name="bitswan-handbook"]');
    const parts = ((el && el.getAttribute('content')) || '').split(',').map(s => s.trim()).filter(Boolean);
    return { html: parts.indexOf('html') !== -1, pdf: parts.indexOf('pdf') !== -1 };
  } catch (e) { return { html: false, pdf: false }; }
}

function HandbookView() {
  const formats = handbookFormats();
    ...
    <div>
      ...
      <div style={card}>
        ...
        {formats.html || formats.pdf ? (
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', background: AC.surface2,
            border: `1px solid ${AC.border}`, borderRadius: 10, padding: '14px 16px', maxWidth: 620 }}>
            <AIcon name="info" size={16} color={AC.muted} />
            <div style={{ fontSize: 13, color: AC.muted, lineHeight: '19px' }}>
              <span style={{ fontWeight: 700, color: AC.fg }}>This build doesn't carry the handbook.</span>{' '}
              The handbook is rendered from a live walkthrough of the product and baked into release
              builds of the automation server. This server runs a build made without one, so there is
              nothing to open here. Update to a released version to get it.
            </div>
          </div>
        )}
      </div>
