...
describe('Handbook view links (issue #45)', () => {
  // The daemon declares which handbook artifacts the running build actually
  // carries; without that declaration there is nothing to link to.
  function declareHandbook(content) {
    const el = document.createElement('meta');
    el.setAttribute('name', 'bitswan-handbook');
    el.setAttribute('content', content);
    document.head.appendChild(el);
  }
  afterEach(() => {
    document.querySelectorAll('meta[name="bitswan-handbook"]').forEach((el) => el.remove());
  });
  ...
  // The handbook is generated by the E2E walkthrough and baked in by the
  // release build, so a build can carry none. Offering the buttons anyway is a
  // dead link: the daemon can only answer 404. Say so instead.
  it('offers no links and explains itself when the build carries no handbook', async () => {
    declareHandbook('');
    setLocation({ pathname: '/handbook' });
    installFetch(fullRoutes());
    render(<App />);
    expect(await screen.findByText(/This build doesn't carry the handbook/)).toBeTruthy();
    expect(screen.queryByText('Read the handbook')).toBeNull();
    expect(screen.queryByText('Download PDF')).toBeNull();
  });

  // Same when the declaration is missing altogether: SPA and daemon ship in one
  // binary, so an absent tag means nothing was declared — never guess a link.
  it('offers no links when the daemon declared nothing', async () => {
    setLocation({ pathname: '/handbook' });
    installFetch(fullRoutes());
    render(<App />);
    expect(await screen.findByText(/This build doesn't carry the handbook/)).toBeTruthy();
  ...
  // A build that carries only one of the two artifacts offers only that one.
  it('offers only the artifacts that exist', async () => {
    declareHandbook('html');
    setLocation({ pathname: '/handbook' });
    installFetch(fullRoutes());
    render(<App />);
