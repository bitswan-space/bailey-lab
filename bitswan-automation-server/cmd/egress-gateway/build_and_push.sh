#!/bin/bash
# Build + publish the egress-gateway image, matching the versioned scheme used by
...
# dockerhub.ResolveEgressGatewayImage(staging) pins.
# Run from the bitswan-automation-server module root (build context = ".").
set -euo pipefail
YEAR=$(date +%Y)
COMMIT_HASH=$(git rev-parse --short HEAD)
IMAGE_TAG="bitswan/egress-gateway-staging"

docker build -t $IMAGE_TAG:latest -t $IMAGE_TAG:$YEAR-${GITHUB_RUN_ID}-git-$COMMIT_HASH -f ./cmd/egress-gateway/Dockerfile .
