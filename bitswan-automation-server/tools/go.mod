// Development tooling lives in its own module ON PURPOSE.
//
// These are linters and formatters — nothing here is needed to COMPILE the
// automation server. While they sat in the main module they were part of its
// module graph, so every `go mod download` pulled them: the gitops,
// egress-gateway, infra-driver and coding-agent images each fetched seven
// linters (and their transitive dependencies) in order to run `go build`. That
// is slow on every build and fragile on a bad day — an E2E bring-up failed
// twice on a proxy.golang.org outage while fetching golangci-lint and
// gotestfmt, neither of which any image needs.
//
// Installing them is `make bootstrap`, which runs go:generate inside THIS
// module. Nothing imports it.
module github.com/bitswan-space/bitswan-workspaces/tools
