...
export class GitopsClient {
  ...
  /** `GET /copies/{name}/behind` — how many commits on main this copy lacks,
   *  in total and per business process (only the BPs that are actually
   *  behind). Cheap: ref comparison inside the bare repos, no fetch. This is
   *  what decides whether the Sync step exists at all, so it is asked on
   *  demand for the ONE copy on screen rather than precomputed for every
   *  copy in the `copies` snapshot. */
  async copyBehind(
    name: string,
  ): Promise<{ ok: boolean; status: number; body: unknown }> {
    const r = await fetch(
      `${this.baseUrl}/copies/${encodeURIComponent(name)}/behind`,
      { headers: { ...this.authHeaders() } },
    );
    let body: unknown = null;
    try {
