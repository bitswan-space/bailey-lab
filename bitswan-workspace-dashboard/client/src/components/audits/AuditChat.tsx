...
export function AuditChat({ bp }: { bp: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const qs = new URLSearchParams({ bp });
      if (token) qs.set('access_token', token);
      socket = new WebSocket(`${proto}//${window.location.host}/ws/audit-agent-chat?${qs}`);
  ...
  const src = api.audits.chatViewUrl(bp);
  return (
    <div className="relative h-full w-full">
      ...
      {!ready && (
        <div className="pointer-events-none absolute inset-x-0 top-0 flex items-center justify-center gap-2 bg-white/85 py-2 text-xs text-muted-foreground">
          {failed ? (
            <>Lost the connection to the audit agent — reload to retry.</>
          ) : (
            <>
              <Loader2 className="size-3.5 animate-spin" aria-hidden /> Starting the audit agent…
            </>
          )}
        </div>
      )}
